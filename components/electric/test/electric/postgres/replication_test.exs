defmodule Electric.Postgres.ReplicationTest do
  ...
  describe "migrate/2" do
    test "updates the schema and returns a valid protcol message" do
      ...
      assert {:ok, schema, msg} = Replication.migrate(schema, version, stmt)
    ...
    test "multiple alter table cmds" do
      ...
      assert {:ok, schema, _msg} = Replication.migrate(schema, version, stmt)
      ...
      assert {:ok, _schema, msg} = Replication.migrate(schema, version, stmt)
