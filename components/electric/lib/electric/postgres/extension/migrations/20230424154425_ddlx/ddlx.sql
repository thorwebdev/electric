-- preprocessed by pgsqlpp for PG=15
--
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_describe(
  IN regclass,
  OUT ord smallint,
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_get_constraints(
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_get_rules(
  regclass default null,
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_get_triggers(
  regclass default null,
  OUT oid oid,
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_get_indexes(
  regclass default null,
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_get_functions(
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_banner(
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_comment(oid)
 RETURNS text LANGUAGE sql AS $function$
 with obj as (select * from #schema#.ddlx_identify($1))
 select format(
...
/*
CREATE OR REPLACE FUNCTION #schema#.ddlx_grants(oid) RETURNS text
  LANGUAGE sql AS $function$ select null::text $function$;
*/
CREATE OR REPLACE FUNCTION #schema#.ddlx_create(oid, text[] default '{}') RETURNS text
  LANGUAGE sql AS $function$ select null::text $function$;

---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_alter_owner(oid)
 RETURNS text LANGUAGE sql AS $function$
 with obj as (select * from #schema#.ddlx_identify($1))
 select
   case
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_table(regclass, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
  with obj as (select * from #schema#.ddlx_identify($1))
  select   
    array_to_string(array[
  ...
  then ' PARTITION OF ' || (SELECT string_agg(i.inhparent::regclass::text,',')
    ...
    ' (' ||coalesce(E'\n' ||
      (SELECT string_agg('    '||definition,E',\n')
         FROM #schema#.ddlx_describe($1) WHERE is_local)||E'\n','') || ')'
    end
  end
  ,
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_view(regclass, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
 select 
 'CREATE '||
  case relkind 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_alter_sequence(regclass)
 RETURNS text LANGUAGE sql AS $function$
 with obj as (select * from #schema#.ddlx_identify($1))
 select 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_sequence(regclass, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
 with obj as (select * from #schema#.ddlx_identify($1)),
 seq as (
...
-- || #schema#.ddlx_alter_sequence($1)
   from obj;
$function$  strict;

---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_create_type_shell(regtype)
 RETURNS text LANGUAGE sql AS $function$
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_type_base(regtype)
 RETURNS text LANGUAGE sql AS $function$
...
---------------------------------------------------
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_type_range(regtype)
 RETURNS text  LANGUAGE sql AS $function$
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_type_enum(regtype)
 RETURNS text LANGUAGE sql AS $function$
with
ee as (
 select 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_type_domain(regtype)
 RETURNS text LANGUAGE sql AS $function$
with
cc as (
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_index(regclass,ddlx_options text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
 with ii as (
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_class(regclass, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
 with obj as (select * from #schema#.ddlx_identify($1)),
 ...
 select #schema#.ddlx_banner(obj.name,obj.sql_kind,obj.namespace,obj.owner) 
  ||
 case 
  when obj.sql_kind in ('VIEW','MATERIALIZED VIEW') then #schema#.ddlx_create_view($1,$2)  
  when obj.sql_kind in ('TABLE','TYPE','FOREIGN TABLE') then #schema#.ddlx_create_table($1,$2)
  when obj.sql_kind in ('SEQUENCE') then #schema#.ddlx_create_sequence($1,$2)
  when obj.sql_kind in ('INDEX') then #schema#.ddlx_create_index($1,$2)
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_alter_table_storage(regclass)
 RETURNS text LANGUAGE sql AS $function$
with 
obj as (select * from #schema#.ddlx_identify($1)),
d as (select * from #schema#.ddlx_describe($1) order by name),
cs as (
  select 
    coalesce(
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_alter_table_settings(regclass)
 RETURNS text LANGUAGE sql AS $function$
with 
obj as (select * from #schema#.ddlx_identify($1)),
ob as (
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_default(oid)
 RETURNS text LANGUAGE sql AS $function$
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_drop_default(oid)
 RETURNS text LANGUAGE sql AS $function$
 ...
 with cs as (
  select
   ...
   E'\n  ' || constraint_definition as sql
    from #schema#.ddlx_get_constraints($1)
   where is_local
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_constraint(oid)
 RETURNS text LANGUAGE sql AS $function$
 select format(
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_drop_constraint(oid)
 RETURNS text LANGUAGE sql AS $function$
 select format(
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_rules(regclass)
 RETURNS text LANGUAGE sql AS $function$
  select coalesce(string_agg(rule_definition,E'\n')||E'\n\n','')
    from #schema#.ddlx_get_rules()
   where regclass = $1
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_rule(oid)
 RETURNS text LANGUAGE sql AS $function$
  select case
         when ev_type='1' and r.rulename='_RETURN'
         then #schema#.ddlx_create_class(c.oid)
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_trigger(oid)
 RETURNS text LANGUAGE sql AS $function$
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_triggers(regclass)
 RETURNS text LANGUAGE sql AS $function$
 with tg as (
  select #schema#.ddlx_create_trigger(oid) as sql 
 from #schema#.ddlx_get_triggers($1) where is_constraint is null
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_drop_trigger(oid)
 RETURNS text LANGUAGE sql AS $function$
 select format(
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_indexes(regclass,ddlx_options text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
 with
 ii as (select * from #schema#.ddlx_get_indexes($1) order by name),
 a as (
  select coalesce(string_agg(#schema#.ddlx_create_index(oid,$2),'') || E'\n' , E'') as ddl_idx
 ...
 ,
 b as (
  select coalesce(string_agg(#schema#.ddlx_create(oid),'' order by oid)||E'\n', '') as ddl_stx
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_aggregate(regproc)
 RETURNS text LANGUAGE sql AS $function$ 
  with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_function(regproc)
 RETURNS text LANGUAGE sql AS $function$ 
 with obj as (select * from #schema#.ddlx_identify($1))
 select
  #schema#.ddlx_banner(sql_identifier,obj.sql_kind,namespace,owner) ||
  case obj.sql_kind
    when 'AGGREGATE' then #schema#.ddlx_create_aggregate($1)
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_grants(regrole) 
 RETURNS text LANGUAGE sql AS $function$
with 
q as (
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_alter_role_auth(oid)
 RETURNS text LANGUAGE sql AS $function$ 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_role(regrole)
 RETURNS text
 LANGUAGE sql
AS $function$ 
with 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_alter_role(regrole)
 RETURNS text
 LANGUAGE sql
AS $function$ 
with 
q1 as (
 select 
   ...
   case when has_table_privilege('pg_catalog.pg_authid'::regclass, 'select')
        then #schema#.ddlx_alter_role_auth(a.oid)||E';\n'
   end,
...
---------------------------------------------------
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_event_trigger(oid)
 RETURNS text LANGUAGE sql AS $function$ 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_foreign_data_wrapper(oid)
 RETURNS text LANGUAGE sql AS $function$ 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_server(oid, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$ 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_user_mapping(oid, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$ 
 with obj as (select * from #schema#.ddlx_identify($1))
 select
    'CREATE USER MAPPING '
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_policy(oid)
 RETURNS text LANGUAGE sql AS $function$ 
 with obj as (select * from #schema#.ddlx_identify($1)),
 pol1 as (
 SELECT
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_transform(oid)
 RETURNS text LANGUAGE sql AS $function$ 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_publication(oid)
 RETURNS text LANGUAGE sql AS $function$ 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_subscription(oid)
 RETURNS text LANGUAGE sql AS $function$ 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_grants_columns(regclass) 
 RETURNS text LANGUAGE sql AS $function$
with
obj as (select * from #schema#.ddlx_identify($1)),
e as (
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_grants(regclass) 
 RETURNS text LANGUAGE sql AS $function$
 with 
 obj as (select * from #schema#.ddlx_identify($1)),
 a   as (
...
select coalesce(string_agg(a.ddl,''),'')||
       #schema#.ddlx_grants_columns($1) from a
$function$  strict;

---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_grants(regproc) 
 RETURNS text LANGUAGE sql AS $function$
 with obj as (select * from #schema#.ddlx_identify($1))
 select
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_grants(oid) 
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1)),
a as (
...
select case obj.classid
       when 'pg_class'::regclass then #schema#.ddlx_grants(obj.oid::regclass)
       when 'pg_proc'::regclass then #schema#.ddlx_grants(obj.oid::regproc)
       when 'pg_roles'::regclass then #schema#.ddlx_grants(obj.oid::regrole)
       else c.grants
       end
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_language(oid)
 RETURNS text LANGUAGE sql AS $function$ 
...
create or replace function #schema#.ddlx_get_dependants(
 in oid, 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_apropos(
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_alter_table_rls(regclass)
 RETURNS text LANGUAGE sql AS $function$
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_operator(regoper)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
select format(
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_operator(regoperator)
 RETURNS text LANGUAGE sql AS $function$
   select #schema#.ddlx_create_operator($1::regoper)
$function$  strict;
---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_create_text_search_config(regconfig)
 RETURNS text LANGUAGE sql AS $function$
with cfg as (select * from pg_ts_config where oid = $1),
     prs as (select * from #schema#.ddlx_identify(
              (select p.oid 
                 from pg_ts_parser p
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_text_search_dict(regdictionary)
 RETURNS text LANGUAGE sql AS $function$
with dict as (select * from pg_ts_dict where oid = $1),
     tmpl as (select * from #schema#.ddlx_identify(
              (select t.oid 
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_text_search_parser(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_text_search_template(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_cast(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_collation(oid, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_conversion(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_tablespace(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_database(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_alter_database(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_access_method(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_operator_class(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_operator_family(oid)
 RETURNS text LANGUAGE sql AS $function$
with obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_amproc(oid)
 RETURNS text LANGUAGE sql AS $function$
with 
a as (
 select *
   ...
   join pg_am am on (am.oid=opf.opfmethod)
  where amp.oid = $1),
f as (select * from #schema#.ddlx_identify((select amprocfamily from a)) ),
obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_drop_amproc(oid)
 RETURNS text LANGUAGE sql AS $function$
with 
a as (
 select *
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_amop(oid)
 RETURNS text LANGUAGE sql AS $function$
with 
a as (
 select *
   ...
   join pg_am am on am.oid=opf.opfmethod
  where amp.oid = $1),
f as (select * from #schema#.ddlx_identify((select amopfamily from a)) ),
obj as (select * from #schema#.ddlx_identify($1))
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_drop_amop(oid)
 RETURNS text LANGUAGE sql AS $function$
with 
a as (
 select *
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_schema(regnamespace, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_extension(oid, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_create_type(regtype, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
   select #schema#.ddlx_create_class(c.oid::regclass,$2) -- type
     from pg_type t
     join pg_class c on (c.oid=t.typrelid)
    where t.oid = $1 and t.typtype = 'c' and c.relkind = 'c'
    union
   select #schema#.ddlx_create(c.oid::regclass, $2) -- table, etc
     from pg_type t
     join pg_class c on (c.oid=t.typrelid)
    where t.oid = $1 and t.typtype = 'c' and c.relkind <> 'c'
    union
   select case t.typtype
          when 'e' then #schema#.ddlx_create_type_enum(t.oid)
          when 'd' then #schema#.ddlx_create_type_domain(t.oid)
          when 'b' then #schema#.ddlx_create_type_base(t.oid)
          when 'r' then #schema#.ddlx_create_type_range(t.oid)
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_definitions(
...
with obj as (select * from #schema#.ddlx_identify($1))
  select 
    ...
    case obj.classid
    when 'pg_class'::regclass          then #schema#.ddlx_create_class(oid::regclass,$2)
    when 'pg_type'::regclass           then #schema#.ddlx_create_type(oid::regtype,$2)
    when 'pg_proc'::regclass           then #schema#.ddlx_create_function(oid::regproc)
    when 'pg_operator'::regclass       then #schema#.ddlx_create_operator(oid::regoper)
    when 'pg_opfamily'::regclass       then #schema#.ddlx_create_operator_family(oid)
    when 'pg_rewrite'::regclass        then #schema#.ddlx_create_rule(oid)
    when 'pg_ts_config'::regclass      then #schema#.ddlx_create_text_search_config(oid::regconfig)
    when 'pg_ts_dict'::regclass        then #schema#.ddlx_create_text_search_dict(oid::regdictionary)
    when 'pg_ts_parser'::regclass      then #schema#.ddlx_create_text_search_parser(oid)
    when 'pg_ts_template'::regclass    then #schema#.ddlx_create_text_search_template(oid)
    when 'pg_database'::regclass       then #schema#.ddlx_create_database(oid)
    when 'pg_constraint'::regclass     then #schema#.ddlx_create_constraint(oid)
    when 'pg_trigger'::regclass        then #schema#.ddlx_create_trigger(oid)
    when 'pg_attrdef'::regclass        then #schema#.ddlx_create_default(oid)
    when 'pg_foreign_data_wrapper'::regclass then #schema#.ddlx_create_foreign_data_wrapper(oid)
    when 'pg_foreign_server'::regclass then #schema#.ddlx_create_server(oid,$2)
    when 'pg_user_mapping'::regclass   then #schema#.ddlx_create_user_mapping(oid,$2)
    when 'pg_cast'::regclass           then #schema#.ddlx_create_cast(oid)
    when 'pg_collation'::regclass      then #schema#.ddlx_create_collation(oid,$2)
    when 'pg_conversion'::regclass     then #schema#.ddlx_create_conversion(oid)
    when 'pg_language'::regclass       then #schema#.ddlx_create_language(oid)
    when 'pg_opclass'::regclass        then #schema#.ddlx_create_operator_class(oid)
    when 'pg_extension'::regclass      then #schema#.ddlx_create_extension(oid,$2)
    when 'pg_roles'::regclass          then #schema#.ddlx_create_role(oid::regrole)
    when 'pg_namespace'::regclass      then #schema#.ddlx_create_schema(oid::regnamespace,$2)
    when 'pg_tablespace'::regclass     then #schema#.ddlx_create_tablespace(oid)
    when 'pg_event_trigger'::regclass  then #schema#.ddlx_create_event_trigger(oid)
    when 'pg_amproc'::regclass         then #schema#.ddlx_create_amproc(oid)
    when 'pg_amop'::regclass           then #schema#.ddlx_create_amop(oid)
    when 'pg_policy'::regclass         then #schema#.ddlx_create_policy(oid)
    when 'pg_transform'::regclass      then #schema#.ddlx_create_transform(oid)
    when 'pg_am'::regclass             then #schema#.ddlx_create_access_method(oid)
    when 'pg_statistic_ext'::regclass  then pg_get_statisticsobjdef(oid)||E';\n' 
    when 'pg_publication'::regclass    then #schema#.ddlx_create_publication(oid)
    when 'pg_subscription'::regclass   then #schema#.ddlx_create_subscription(oid)
    else
      case
    ...
    #schema#.ddlx_comment(oid) as comment,
    #schema#.ddlx_alter_owner(oid) as owner,
    #schema#.ddlx_alter_table_storage(oid) as storage,
    #schema#.ddlx_alter_table_defaults(oid) as defaults,
    case obj.sql_kind
      when 'ROLE' THEN #schema#.ddlx_alter_role(oid)
      when 'DATABASE' THEN  #schema#.ddlx_alter_database(oid)
      when 'SEQUENCE' THEN #schema#.ddlx_alter_sequence(oid)
      else #schema#.ddlx_alter_table_settings(oid)
    end as settings,
--    #schema#.ddlx_alter_table_settings(oid) as settings,
    #schema#.ddlx_create_constraints(oid) as constraints,
    #schema#.ddlx_create_indexes(oid, options) as indexes,
    #schema#.ddlx_create_triggers(oid) as triggers,
    #schema#.ddlx_create_rules(oid) as rules,
     #schema#.ddlx_alter_table_rls(oid) as rls,
     #schema#.ddlx_grants(oid) as grants
    from obj
$function$  strict;

---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_alter(oid, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
with 
obj as (select * from #schema#.ddlx_identify($1)),
parts as (select * from #schema#.ddlx_definitions($1,$2))
...
COMMENT ON FUNCTION #schema#.ddlx_alter(oid, text[]) 
     IS 'Get SQL ALTER statement for any object by object id (post-data)';
     
---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_createonly(oid, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
with 
obj as (select * from #schema#.ddlx_identify($1)),
parts as (select * from #schema#.ddlx_definitions($1,$2))
...
COMMENT ON FUNCTION #schema#.ddlx_createonly(oid, text[]) 
     IS 'Get SQL CREATE statement for any object by object id (pre-data)';

---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_create(oid, text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
with 
obj as (select * from #schema#.ddlx_identify($1)),
parts as (select * from #schema#.ddlx_definitions($1,$2))
...
COMMENT ON FUNCTION #schema#.ddlx_create(oid, text[]) 
     IS 'Get SQL CREATE statement for any object by object id. Includes constraints, triggers, indexes...';
     
---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_drop(oid,ddlx_options text[] default '{}') 
 RETURNS text LANGUAGE sql AS $function$
 with obj as (select * from #schema#.ddlx_identify($1))
 select 
   case obj.classid
   when 'pg_constraint'::regclass then #schema#.ddlx_drop_constraint(oid)
   when 'pg_trigger'::regclass    then #schema#.ddlx_drop_trigger(oid)
   when 'pg_attrdef'::regclass    then #schema#.ddlx_drop_default(oid)
   when 'pg_amproc'::regclass     then #schema#.ddlx_drop_amproc(oid)
   when 'pg_amop'::regclass       then #schema#.ddlx_drop_amop(oid)
   else
     case
...
COMMENT ON FUNCTION #schema#.ddlx_drop(oid,text[]) 
     IS 'Get SQL DROP statement for any object by object id';
     
---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_script_parts(
...
select row_number() over() as n,
       #schema#.ddlx_drop(gd.objid,$2),
       #schema#.ddlx_create(gd.objid,$2),
       gd.objid
  from #schema#.ddlx_get_dependants($1) gd
...
select #schema#.ddlx_create($1,$2) as ddl_create,
       #schema#.ddlx_drop($1,$2) as ddl_drop,
...
CREATE OR REPLACE FUNCTION #schema#.ddlx_script(oid, ddlx_options text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
select array_to_string(array[
       E'BEGIN;\n',
       ...
       E'-- DEPENDANTS\n\n'||ddl_create_deps,       
       E'END;\n'],E'\n')
  from #schema#.ddlx_script_parts($1,$2)
$function$ strict;

COMMENT ON FUNCTION #schema#.ddlx_script(oid, text[]) 
     IS 'Get SQL DDL script for any object and dependants by object id';

---------------------------------------------------

CREATE OR REPLACE FUNCTION #schema#.ddlx_script(sql_identifier text, ddlx_options text[] default '{}')
 RETURNS text LANGUAGE sql AS $function$
  select case
    when strpos($1,'(')>0 
    then #schema#.ddlx_script(cast($1 as regprocedure)::oid, $2)
    else #schema#.ddlx_script((
         select coalesce(c.oid,t.oid)
           from pg_type t 
...
COMMENT ON FUNCTION #schema#.ddlx_script(text, text[]) 
